...
/* pagesize hack */
extern size_t getpagesize$long(void);

#define ABORT _ZN3lnk4ldbg11printNumberEPKci("\nMemory: abort on line " , __LINE__)
#define malloc_getpagesize getpagesize$long()
...
#define MSPACES 1
#define USE_LOCKS 1 
#define MALLOC_FAILURE_ACTION _ZN3lnk4ldbg11printNumberEPKci("\nsomething is fucked " , __LINE__)
...
int XXX_mspace_has_pointer(mspace msp, void* ptr) {
	...
	if (ok_magic(ms)) {
		...
		while (sp != 0) {
			void* base = sp->base;
			size_t size = sp->size;
			void* end = (char*)((uintptr_t)base + (uintptr_t)size);
			...
			{
				/*
				 * If the address falls between those two, it is 
				 * in the mspace.
				 */
				if (ptr >= base && ptr <= end)
				{
